...
import { generateTechnicalMCQ, generateQuestions } from '../data/questionBank';
...
export default function CandidateCodingEnv() {
  ...
  // Load questions — with live fallback if stored schedule has empty questions (e.g. saved before bug fix)
  ...
  const questions = (() => {
    ...
    // Fallback: regenerate for coding rounds
    const count = Number(activeSchedule.numQuestions) || 5;
    const generated = generateQuestions(activeSchedule.topics || {}, count, true);
    return generated.length > 0 ? generated : generateQuestions({}, count, true);
  })();
    ...
    try {
      ...
      if (SpeechRecognition) {
        ...
        recognition.onresult = (event) => {
          ...
          setMcqAnswers(prev => ({ ...prev, [currentQ?.id]: prev[currentQ?.id] ? prev[currentQ?.id] + finalTranscript : finalTranscript }));
        };
        recognition.start();
  ...
  const handleMalpractice = useCallback((reason) => {
    ...
    if (currentWarnings >= 9) {
      ...
      submitRound(candidateId, driveId, roundId, 0, 0, "0m", {
        ...
        reason: reason,
        warnings: Math.floor(currentWarnings / 3),
  ...
  useEffect(() => {
    ...
    // Violation tracking start times (in ms)
    let noFaceStartTime = 0;
    let multipleFacesStartTime = 0;
    let noFaceTimer = null;
    
    // Smart Engine Moving Averages
    let horizontalQueue = [];
    let verticalQueue = [];
    let headTurnStartTime = 0;
    let headTiltStartTime = 0;

    const initCamera = async () => {
      try {
        stream = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
        streamRef.current = stream; // persist so we can reattach after review page
        if (videoRef.current) {
          ...
          try {
            ...
            if (detections.length === 0) {
              ...
              if (noFaceStartTime === 0) {
                noFaceStartTime = now;
              } else if (now - noFaceStartTime > 2000) {
                setFaceDetected(false); // Only show red indicator after 2 seconds
              }
              
              if (noFaceStartTime > 0 && now - noFaceStartTime > 8000) {
                handleMalpracticeRef.current("AI Proctoring: Face not detected or camera covered. Please look at the screen.");
                noFaceStartTime = 0; // reset to avoid spam
              }
            } else if (detections.length > 1) {
              multipleFacesStartTime = 0; // Don't instantly flash the red dot
              
              if (multipleFacesStartTime === 0) multipleFacesStartTime = now;
              else if (now - multipleFacesStartTime > 2000) {
                 setFaceDetected(false);
              }
              
              if (multipleFacesStartTime > 0 && now - multipleFacesStartTime > 8000) {
              ...
              const horizontalRatio = distRight > 0 ? distLeft / distRight : 1;

              const chin = jaw[8];
              const eyeCenterY = (leftEye[0].y + rightEye[3].y) / 2;
              const distTop = Math.abs(noseTip.y - eyeCenterY);
              const distBottom = Math.abs(noseTip.y - chin.y);
              const verticalRatio = distBottom > 0 ? distTop / distBottom : 1;

              // Smart Engine: Push to queues (multi-frame confirmation)
              horizontalQueue.push(horizontalRatio);
              if (horizontalQueue.length > 8) horizontalQueue.shift();
              
              verticalQueue.push(verticalRatio);
              if (verticalQueue.length > 8) verticalQueue.shift();

              // Confidence Averaging
              const avgHorizontal = horizontalQueue.reduce((a,b)=>a+b, 0) / horizontalQueue.length;
              const avgVertical = verticalQueue.reduce((a,b)=>a+b, 0) / verticalQueue.length;

              // Threshold logic (Extreme ratios smoothed out over ~6 seconds)
              let isHeadTurned = (horizontalQueue.length === 8) && (avgHorizontal < 0.2 || avgHorizontal > 4.0);
              let isHeadTilted = (verticalQueue.length === 8) && (avgVertical < 0.1 || avgVertical > 3.5);

              if (isHeadTurned) {
                if (headTurnStartTime === 0) headTurnStartTime = now;
                else if (now - headTurnStartTime > 8000) {
                  handleMalpracticeRef.current("AI Proctoring: Suspicious head turn detected. Please remain focused on the assessment.");
                  headTurnStartTime = 0;
                  horizontalQueue = [];
                }
              } else {
              ...
              if (isHeadTilted) {
                if (headTiltStartTime === 0) headTiltStartTime = now;
                else if (now - headTiltStartTime > 8000) {
                  handleMalpracticeRef.current("AI Proctoring: Suspicious head tilt detected. Please remain focused on the assessment.");
                  headTiltStartTime = 0;
                  verticalQueue = [];
                }
              } else {
        ...
        }, 800); // Scans every 800ms, but requires continuous duration to trigger
  ...
  useEffect(() => {
    if (submitted || isSubmitting || warningCount >= 4) {
      ...
      setFaceDetected(false); // Stop red indicator blinking after termination
    }
  }, [submitted, isSubmitting, warningCount]);
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
      <div className="bg-[#0f172a] border border-red-500/30 rounded-2xl p-8 max-w-md w-full shadow-2xl text-center shadow-red-500/10">
        ...
        <h2 className="text-2xl font-bold text-white mb-2">
          {warningCount >= 9 ? "Assessment Terminated (3/3 Warnings)" : `Security Warning (${Math.floor(warningCount / 3)}/3 Warnings)`}
        </h2>
        <div className="text-slate-300 mb-6 space-y-2 text-sm">
          <p className="text-xs text-yellow-400 font-bold">Total Security Alerts: {warningCount} / 9</p>
          {warningCount >= 9 ? (
            <>
              <p>Maximum security violations (3 warnings / 9 alerts) reached.</p>
              <p>Assessment has been terminated.</p>
              <p>Your session has been locked.</p>
            </>
          ) : (
            <>
              <p>Unusual activity detected.</p>
              <p>3 alerts constitute 1 warning. 3 warnings (9 alerts) will terminate the exam.</p>
              <p className="text-xs text-red-400 font-bold mt-2">Remaining Warnings: {3 - Math.floor(warningCount / 3)}</p>
              <p className="text-xs text-red-400 font-bold">Alerts until next warning: {3 - (warningCount % 3)}</p>
            </>
          )}
        ...
        <div className="flex flex-col gap-3">
          {warningCount >= 9 ? (
            ...
            <>
              <div className="text-xs text-red-400 font-bold mb-1">Remaining chances: {9 - warningCount}</div>
              <button
                onClick={() => {
                  setShowWarningModal(false);
                }}
  ...
  const CameraPIP = isProctoredRound && (
    <div className={`fixed bottom-6 left-6 z-[9999] bg-black rounded-xl shadow-2xl overflow-hidden w-64 h-48 border-4 transition-colors duration-300 ${faceDetected ? 'border-green-500/80 shadow-green-500/20' : 'border-red-500/80 shadow-red-500/40'
      }`}>
      <video
        ref={videoRef}
        autoPlay
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {currentIdx < questions.length - 1 ? (
                  ...
                  <button
                    ...
                    className="ml-auto flex items-center gap-2 px-6 py-2.5 rounded-xl bg-green-600 hover:bg-green-500 text-white text-sm font-bold transition-all shadow-lg shadow-green-500/20"
                  >
